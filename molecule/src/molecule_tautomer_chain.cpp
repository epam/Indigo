...
TautomerChainFinder::TautomerChainFinder (TautomerSearchContext &context, int h_difference,
   int start_path_number, int n_chains) :
_context(context),
_path_length(0),
_h_difference(h_difference),
_is_zero_bond_present(!context.ring_chain),
_path_number(start_path_number + 2),
_n_chains(n_chains)
{
}
...
_path_length(other._path_length + 1),
_h_difference(other._h_difference * -1),
_path_number(other._path_number),
_start_idx1(other._start_idx1),
_start_idx2(other._start_idx2),
_n_chains(other._n_chains)
{
}
...
int TautomerChainFinder::isFeasiblePair (int n1, int n2, bool &zero_bond, int &arom_bond_idx2, int &bond_type2)
{
   ...
   {
      zero_bond = true;
   } else if (_context.g2.getBondOrder(edge2) == -1 &&
              _context.g2.possibleBondOrder(edge2, BOND_SINGLE))
   {
...
_h_difference(0),
_is_zero_bond_present(!context.ring_chain),
_is_query_bond_present(false),
_is_non_aromatic_bond_present(false),
_path_number(start_path_number + 2),
_core_1(core1),
_core_2(core2),
_tau_bonds_to_match(0),
_prev_n1(-1),
_prev_n2(-1),
_bond_idx1(-1),
_bond_idx2(-1),
...
_h_difference(other._h_difference * -1),
_is_zero_bond_present(other._is_zero_bond_present),
_is_query_bond_present(other._is_query_bond_present),
_is_non_aromatic_bond_present(other._is_non_aromatic_bond_present),
_path_number(other._path_number),
_core_1(other._core_1),
_core_2(other._core_2),
_tau_bonds_to_match(other._tau_bonds_to_match),
_prev_n1(-1),
_prev_n2(-1),
_bond_idx1(-1),
_bond_idx2(-1),
...
int TautomerChainChecker::isFeasiblePair (int n1, int n2, TautomerChainChecker &next1, TautomerChainChecker &next2)
{
   ...
   if (_prev_n1 != -1 && _path_length != 1)
   {
      ...
      for (i = prev_vert2.neiBegin(); i < prev_vert2.neiEnd(); i = prev_vert2.neiNext(i))
      {
         ...
         if ((value_chain > 0 && abs(value_chain - prev_value_chain) == 1) ||
              (_context.g2.getBondOrder(prev_vert2.neiEdge(i)) == -1 &&
               _context.g2.possibleBondOrder(prev_vert2.neiEdge(i), BOND_SINGLE)))
            bond_count2--;
      }
   ...
   if (n1 != -1)
   {
      ...
      if (query_bond && na_bond)
      {
         if (h_diff != 0)
         {
            ...
            if (_context.g1.possibleAtomTotalH(n1, h_count_2 + 1 - h_rep_1) ||
                _context.g1.possibleAtomTotalH(n1, h_count_2 - 1 - h_rep_1))
            {
               if (_context.cb_check_rules == 0 || _context.cb_check_rules(_context, _start_idx1, _start_idx2, n1, n2))
                  rc += 2;
            }
      ...
      if (type > 0 && idx1 < 0)
      {
         ...
         if (beg < 0)
         {
            if (_context.substructure)
            {
               ...
               {
                  qmol.resetAtom(new_idx, QueryMolecule::Atom::und(
                          qmol.releaseAtom(new_idx),
         ...
         if (_context.substructure)
         {
            ...
            new_bond_idx = qmol.addBond(_context.core_2[edge2.beg], _context.core_2[edge2.end],
                    new QueryMolecule::Bond(QueryMolecule::BOND_ORDER, type));
         }
         else
         {
            Molecule &mol = query.asMolecule();
   ...
   if (_prev_n1 != -1)
   {
      ...
      for (int i = vertex.neiBegin(); i < vertex.neiEnd(); i = vertex.neiNext(i))
      {
         ...
         if (abs(_context.chains_2[_context.core_1[_prev_n1]] - _context.chains_2[_context.core_1[nei_idx]]) != 1)
         {
            ...
            if (_context.substructure)
            {
               matched = MoleculeSubstructureMatcher::matchQueryBond(&(g1.asQueryMolecule()).getBond(idx1),
                       g2, idx1, idx2, 0, 0xFFFFFFFFUL);
            }
            else
            {
...
bool TautomerChainChecker::_matchAromBonds (Graph &subgraph, Graph &supergraph,
...
void TautomerChainChecker::_addBond (Graph &subgraph, Graph &supergraph,
                                     int sub_idx, int super_idx, void *userdata)
{
   AromaticityMatcher *am = (AromaticityMatcher *)userdata;
...
int TautomerChainChecker::_embedding (Graph &subgraph, Graph &supergraph,
