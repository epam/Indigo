...
void Molecule3dConstraints::_buildSub (PtrArray<Base> &sub, const PtrArray<Base> &super,
                                       const int *mapping)
{
   ...
   for (int i = _constraints.begin(); i != _constraints.end(); i = _constraints.next(i))
   {
      ...
      switch (base.type)
      {
         ...
         case MC::ANGLE_2PLANES:
         {
            ...
            if ((value < constr.bottom || value > constr.top) &&
                (M_PI - value < constr.bottom || M_PI - value > constr.top))
               return false;
            break;
         }
         ...
         case MC::EXCLUSION_SPHERE:
         {
            ...
            for (i = _target->vertexBegin(); i < _target->vertexEnd();
                 i = _target->vertexNext(i))
            {
               if (allowed[i])
                  continue;
