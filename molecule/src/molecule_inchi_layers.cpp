...
void MainLayerFormula::_collectAtomsCount ()
{
   ...
   for (int v_idx = mol.vertexBegin();
            v_idx != mol.vertexEnd();
            v_idx = mol.vertexNext(v_idx))
   {
...
int MainLayerFormula::compareComponentsAtomsCountNoH (MainLayerFormula &comp1, 
...
int MainLayerFormula::compareComponentsTotalHydrogensCount (MainLayerFormula &comp1, 
                                                            MainLayerFormula &comp2)
{
...
void MainLayerConnections::_linearizeConnectionTable ()
{
            ...
            v_idx = mol.vertexNext(v_idx))
   {
      ...
      nei_array.clear();
      for (int nei_idx = vertex.neiBegin(); 
               nei_idx != vertex.neiEnd();
               nei_idx = vertex.neiNext(nei_idx))
      {
...
int MainLayerConnections::compareMappings (const MoleculeInChIUtils::Mapping &m1, 
...
int MainLayerConnections::compareComponentsConnectionTables (MainLayerConnections &comp1, 
...
void MainLayerConnections::printConnectionTable (Array<char> &result)
{
   ...
   for (int v_idx = cano_mol.vertexBegin(); 
            v_idx != cano_mol.vertexEnd();
            v_idx = cano_mol.vertexNext(v_idx))
   {
   ...
   enum { PRINT_BRACKET = -1, PRINT_COMMA = -2 };
   ...
   while (vertex_stack.size() != 0)
   {
      ...
      for (int nei_idx = cur_vertex.neiBegin(); 
               nei_idx != cur_vertex.neiEnd();
               nei_idx = cur_vertex.neiNext(nei_idx))
      {
...
int HydrogensLayer::compareComponentsHydrogens (HydrogensLayer &comp1, 
...
int HydrogensLayer::compareMappings (MoleculeInChIUtils::Mapping &m1, 
...
int CisTransStereochemistryLayer::compareMappings (const MoleculeInChIUtils::Mapping &m1, 
                                                   const MoleculeInChIUtils::Mapping &m2)
{
   Molecule &mol = _getMolecule();
...
int CisTransStereochemistryLayer::compareComponents (
   CisTransStereochemistryLayer &comp1, 
...
int  TetrahedralStereochemistryLayer::compareMappings (const MoleculeInChIUtils::Mapping &m1, 
                                                       const MoleculeInChIUtils::Mapping &m2)
{
   Molecule &mol = _getMolecule();
...
int TetrahedralStereochemistryLayer::_getMappingSign (const MoleculeStereocenters &stereocenters, 
...
int TetrahedralStereochemistryLayer::compareComponentsEnantiomers (
   TetrahedralStereochemistryLayer &comp1, 
...
int TetrahedralStereochemistryLayer::compareComponents (
   TetrahedralStereochemistryLayer &comp1, 
   TetrahedralStereochemistryLayer &comp2)
{
   return 0;
