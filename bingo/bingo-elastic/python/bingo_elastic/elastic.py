    ...
    async def filter(
        ...
        **kwargs,
        ...
        Args:
            ...
            limit: Maximum number of records to return. Since some results will be filtered out by
                the postprocess actions, request to Elasticsearch server will use page_size limitation.
        ...
        Notes:
            ...
            See: https://www.elastic.co/docs/reference/elasticsearch/rest-apis/paginate-search-results#scroll-search-results
        """
            ...
            while limit > 0:
                if not scroll_id:
                    res = await self.el_client.search(
                        index=self.index_name, body=query, scroll=keep_alive
                    )
                else:
