...
def test_empty_create(indigo_fixture):
    ...
    mol_record = IndigoRecordMolecule(indigo_object=mol)
    assert len(mol_record.sub_fingerprint) == 0
    assert len(mol_record.sim_fingerprint) == 0
    assert len(mol_record.cmf.split()) == 5
...
def test_create_reaction(
    ...
    for reactant in reaction.iterateReactants():
        reactant.aromatize()
        test_smiles.append(reactant.canonicalSmiles())
