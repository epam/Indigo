...
drop index IX_bingo_test_index
create index IX_bingo_test_index on test_table using bingo_idx (a bingo_ops)
...
create index IX_btest_shadow_index on btest_shadow(idx)
...
select bingoGetIndexStructuresCount('IX_bingo_test_index'::regclass::oid)
...
create index IX_j_index1 on join_idx_shadow_hash (b_id, ex_hash)
create index IX_j_index2 on join_idx_shadow_hash (b_id, f_count)
create index IX_j_index3 on join_idx_shadow_hash (b_id, ex_hash, f_count)
create index IX_j_index4 on join_idx_shadow_hash ( ex_hash, f_count)

create index IX_j_index5 on join_idx_shadow_hash using hash (f_count)
create index IX_j_index6 on join_idx_shadow_hash (b_id)

drop index IX_j_index1
drop index IX_j_index2
drop index IX_j_index3
drop index IX_j_index4
drop index IX_j_index5
drop index IX_j_index6
...
drop index chemistry.IX_bingo_compound_index
CREATE INDEX IX_bingo_compound_index ON chemistry.compound USING bingo_idx (structure bingo.molecule);
