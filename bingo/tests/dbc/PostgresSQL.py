...
class Postgres(SQLAdapter):
    ...
    def checkmolecule(self, molecule):
        query_sql = "SELECT {bingo_schema}.checkmolecule(" "%(query_entity)s)"
    ...
    def compactmolecule(self, molecule):
        query_sql = (
            "SELECT md5({bingo_schema}.compactmolecule("
    ...
    def fingerprint(self, molecule, options):
        query_sql = (
            "SELECT md5({bingo_schema}.fingerprint("
            "%(query_entity)s, '{options}'))"
        )
    ...
    def inchi(self, molecule, options="", inchikey=False):
        query_sql = (
            "SELECT {bingo_schema}.inchi(" "%(query_entity)s, '{options}')"
        )
        if inchikey:
            query_sql = (
                "SELECT {bingo_schema}.inchikey({bingo_schema}."
    ...
    def mass(self, molecule, options):
        query_sql = (
            "SELECT {bingo_schema}.getWeight(" "%(query_entity)s, '{options}')"
        )
        return self.query_row(query_sql, molecule, options=options)
    ...
    def compactreaction(self, reaction, options="0"):
        query_sql = (
            "SELECT md5({bingo_schema}.compactreaction("
    ...
    def rfingerprint(self, reaction, options=""):
        query_sql = (
            "SELECT md5({bingo_schema}.rfingerprint("
            "%(query_entity)s, '{options}'))"
        )
        return self.query_row(query_sql, reaction, options=options)
