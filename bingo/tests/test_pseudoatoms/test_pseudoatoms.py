...
class TestPseudoatoms:
    @pytest.mark.parametrize(
        "query_id, expected", query_cases("pseudoatoms", "exact()")
    )
    ...
    @pytest.mark.parametrize(
        "query_id, expected", query_cases("pseudoatoms", "exact(STE)")
    )
    ...
    @pytest.mark.parametrize(
        "query_id, expected",
        query_cases("pseudoatoms", "similarity(tanimoto, 0.7, 1)"),
    )
    def test_pseudoatoms_similarity_tanimoto(
    ...
    @pytest.mark.parametrize(
        "query_id, expected", query_cases("pseudoatoms", "substructure()")
    )
