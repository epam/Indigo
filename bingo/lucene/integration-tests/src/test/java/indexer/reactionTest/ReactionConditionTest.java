...
public class ReactionConditionTest extends ReactionBaseTest {
    ...
    public void testExactMatchesSearch() throws Exception {
        ...
        addDocuments(variousTextValues);
    ...
    public void testReactionSubstructureSearch() throws Exception {
        ...
        addDocuments(variousReactTextValues);
    ...
    public void testSimilarSearch() throws Exception {
        List<Map<String, Object>> result = new LinkedList<>();
        BeforeGroup<TestSchema> query = testCollection.find()
                .filter(REACT.unsafeIsSimilarTo(RARE_REACTION))
                .limit(2);
    ...
    private void benchmarkReaction(String reaction, int limit) throws Exception {
        long before = System.currentTimeMillis();
    ...
    private void addDocuments(String[] values) throws Exception {
        try (SolrUploadStream uploadStream = testCollection.uploadStream()) {
            for (String variousTextValue : values) {
