...
bool BingoFingerprints::screenPart_Next (OracleEnv &env, Screening &screening)
{
   ...
   if (screening.query_bit_idx == 0)
   {
      ...
      if (screening.part != _part_adding)
         screening.bits_lob->read(query_bit * _chunk_qwords * 8 + screening.start_offset,
            (char *)screening.fp_final.ptr() + screening.start_offset, read_bytes_count);
      else
   ...
   else
   {
      ...
      {
         if (screening.part != _part_adding)
            screening.bits_lob->read(query_bit * _chunk_qwords * 8 +
                                    screening.start_offset, (char *)fp_inter.ptr() + screening.start_offset, read_bytes_count);
         else
...
bool BingoFingerprints::countOnes_Next (OracleEnv &env, Screening &screening)
{
   ...
   if (screening.part != _part_adding)
      screening.bits_lob->read(screening.query_ones[screening.query_bit_idx] * _chunk_qwords * 8,
                     (char *)fp.ptr(), used_qwords * 8);
   else
      memcpy(fp.ptr(), _pending_bits.ptr() + screening.query_ones[screening.query_bit_idx] *
                                             _chunk_qwords, used_qwords * 8);
...
void BingoFingerprints::validateForUpdate (OracleEnv &env)
{
   OracleStatement statement(env);
   OracleLOB 
      counters_lob(env), mapping_lob(env), 
      bit_starts_lob(env), bit_ends_lob(env),
      bits_lob(env);
   int used, part = -1;

   statement.append("SELECT used, counters, mapping, bit_starts, bit_ends, part, bits FROM %s ORDER BY part DESC",
      _table_name.ptr());
...
void BingoFingerprints::validate (OracleEnv &env)
{
   ...
   statement.append("SELECT used, counters, mapping, bit_starts, bit_ends FROM %s ORDER BY part", 
      _table_name.ptr());
   statement.prepare();
   statement.defineIntByPos(1, &used);
   statement.defineBlobByPos(2, counters_lob);
...
int BingoFingerprints::countOracleBlocks (OracleEnv &env)
{
   ...
   if (!OracleStatement::executeSingleInt(res, env, "select sum(length(bits)) / %d from %s",
           _chunk_qwords * 8, _table_name.ptr()))
      return 0;
...
int BingoFingerprints::getTotalCount (OracleEnv &env)
{
   ...
   {
      OracleStatement::executeSingleInt(_total_count_cached, env,
              "SELECT sum(used) FROM %s", _table_name.ptr());
   }
...
void BingoFingerprints::_optimizePendingBlock ()
{
   ...
   QS_DEF(Array<int>, counters_order);
