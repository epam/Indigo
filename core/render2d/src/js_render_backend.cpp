...
EM_JS(void, js_rb_init, (int w, int h, int mode), {
    Module._rb = {
        w: w, h: h, mode: mode,
        // State
        s: { ctm:[1,0,0,1,0,0], r:0, g:0, b:0, a:1, lw:1, lj:'miter', dash:[], doff:0,
             fsz:12, ffam:'Noto Sans', fbold:false, fital:false },
        stack: [],
        // Path
        pd: '',
        cx: 0, cy: 0,
        ...
        bbx0: 1e9, bby0: 1e9, bbx1: -1e9, bby1: -1e9,
        // SVG output
        elems: [],
        defs: '',
        gradCnt: 0,
...
// -- Path operations --
EM_JS(void, js_rb_beginPath, (), { var r=Module._rb; r.pd=''; r.pathEmpty=true; });
EM_JS(void, js_rb_closePath, (), { Module._rb.pd+='Z '; });
EM_JS(void, js_rb_moveTo, (float x, float y), {
    var r=Module._rb; r.pd+='M'+x+' '+y+' '; r.cx=x; r.cy=y; r.pathEmpty=false;
});
EM_JS(void, js_rb_lineTo, (float x, float y), {
    var r=Module._rb; r.pd+='L'+x+' '+y+' '; r.cx=x; r.cy=y; r.pathEmpty=false;
});
EM_JS(void, js_rb_curveTo, (float x1,float y1,float x2,float y2,float x3,float y3), {
    var r=Module._rb; r.pd+='C'+x1+' '+y1+' '+x2+' '+y2+' '+x3+' '+y3+' ';
    r.cx=x3; r.cy=y3; r.pathEmpty=false;
});
EM_JS(void, js_rb_arc, (float cx,float cy,float rad,float a0,float a1,int ccw), {
    ...
    if (r.pathEmpty) { r.pd += 'M'+sx+' '+sy+' '; r.pathEmpty=false; }
    else { r.pd += 'L'+sx+' '+sy+' '; }
    // Use SVG arc command
    var da = a1 - a0;
...
EM_JS(void, js_rb_emitPath, (int doFill, int doStroke), {
    ...
    if (nums) {
        for (var i=0; i+1<nums.length; i+=2) {
            ...
            if(tx<r.bbx0)r.bbx0=tx; if(ty<r.bby0)r.bby0=ty;
            if(tx>r.bbx1)r.bbx1=tx; if(ty>r.bby1)r.bby1=ty;
        }
    }
    var e = '<path d="'+r.pd+'" transform="matrix('+m[0]+','+m[1]+','+m[2]+','+m[3]+','+m[4]+','+m[5]+')"';
    if (doFill) {
        var fc = s.gradId ? 'url(#'+s.gradId+')' :
            'rgb('+Math.round(s.r*255)+','+Math.round(s.g*255)+','+Math.round(s.b*255)+')';
        e += ' fill="'+fc+'"';
        if (s.a < 1 && !s.gradId) e += ' fill-opacity="'+s.a+'"';
    } else {
        e += ' fill="none"';
    }
    if (doStroke) {
        e += ' stroke="rgb('+Math.round(s.r*255)+','+Math.round(s.g*255)+','+Math.round(s.b*255)+')"';
        if (s.a < 1) e += ' stroke-opacity="'+s.a+'"';
        e += ' stroke-width="'+s.lw+'" stroke-linejoin="'+s.lj+'"';
        if (s.dash.length) e += ' stroke-dasharray="'+s.dash.join(',')+'" stroke-dashoffset="'+s.doff+'"';
    } else if (!doFill) {
        e += ' stroke="none"';
    }
    e += '/>';
...
EM_JS(void, js_rb_setColor, (float cr,float cg,float cb,float ca), {
    var s=Module._rb.s; s.r=cr; s.g=cg; s.b=cb; s.a=ca; s.gradId=null;
});
EM_JS(void, js_rb_setLineWidth, (float w), { Module._rb.s.lw=w; });
EM_JS(void, js_rb_setLineJoin, (int j), {
    Module._rb.s.lj = ['miter','round','bevel'][j] || 'miter';
});
EM_JS(void, js_rb_setDash, (const double* p, int n, double off), {
    var s=Module._rb.s; s.dash=[];
    for(var i=0;i<n;i++) s.dash.push(HEAPF64[(p>>3)+i]);
    s.doff=off;
});
...
EM_JS(void, js_rb_save, (), {
    var r=Module._rb, s=r.s;
    r.stack.push({ctm:s.ctm.slice(), r:s.r, g:s.g, b:s.b, a:s.a,
        lw:s.lw, lj:s.lj, dash:s.dash.slice(), doff:s.doff,
        fsz:s.fsz, ffam:s.ffam, fbold:s.fbold, fital:s.fital, gradId:s.gradId});
});
EM_JS(void, js_rb_restore, (), {
    var r=Module._rb;
    if(r.stack.length) r.s = r.stack.pop();
});
...
EM_JS(void, js_rb_mmul, (double A,double B,double C,double D,double E,double F), {
    ...
    m[4]=a*E+c*F+e; m[5]=b*E+d*F+f;
});

EM_JS(void, js_rb_setMatrix, (double a,double b,double c,double d,double e,double f), {
    var m=Module._rb.s.ctm; m[0]=a;m[1]=b;m[2]=c;m[3]=d;m[4]=e;m[5]=f;
});
EM_JS(void, js_rb_getMatrix, (double* out), {
    var m=Module._rb.s.ctm;
    HEAPF64[(out>>3)]=m[0]; HEAPF64[(out>>3)+1]=m[1];
    HEAPF64[(out>>3)+2]=m[2]; HEAPF64[(out>>3)+3]=m[3];
    HEAPF64[(out>>3)+4]=m[4]; HEAPF64[(out>>3)+5]=m[5];
});

// -- Text --
EM_JS(void, js_rb_setFont, (const char* fam, float sz, int bold, int ital), {
    var s=Module._rb.s; s.ffam=UTF8ToString(fam); s.fsz=sz; s.fbold=!!bold; s.fital=!!ital;
});
EM_JS(void, js_rb_fillText, (const char* text, float x, float y), {
    var r=Module._rb; var s=r.s; var m=s.ctm;
    ...
    if(tx<r.bbx0)r.bbx0=tx; if(ty-th<r.bby0)r.bby0=ty-th;
    if(tx+tw>r.bbx1)r.bbx1=tx+tw; if(ty>r.bby1)r.bby1=ty;
    ...
    if(s.fbold) e += ' font-weight="bold"';
    if(s.fital) e += ' font-style="italic"';
    if(s.a<1) e += ' fill-opacity="'+s.a+'"';
...
EM_JS(float, js_rb_measureWidth, (const char* text, float fontSize), {
    ...
    for (var i=0; i<t.length; i++) {
        ...
        if (ch >= 48 && ch <= 57) w += 0.60; // digits
        else if (ch >= 65 && ch <= 90) w += 0.72; // uppercase
        else if (ch === 77 || ch === 87) w += 0.88; // M, W
        else if (ch === 73 || ch === 108) w += 0.33; // I, l
        else if (ch >= 97 && ch <= 122) w += 0.55; // lowercase
        else if (ch === 32) w += 0.30; // space
        else if (ch === 43 || ch === 45) w += 0.55; // +, -
        else w += 0.60;
    }
    return w * fontSize;
});
...
// -- Gradient --
EM_JS(void, js_rb_setGradient, (float x0,float y0,float x1,float y1,
...
EM_JS(int, js_rb_finalize, (int mode), {
    ...
    var vx = 0; var vy = 0; var vw = r.w; var vh = r.h;
    if (r.bbx1 > r.bbx0 && r.bby1 > r.bby0) {
        var pad = 2;
        vx = r.bbx0 - pad; vy = r.bby0 - pad;
        vw = r.bbx1 - r.bbx0 + 2*pad;
        vh = r.bby1 - r.bby0 + 2*pad;
    }
    var svg = '<svg xmlns="http://www.w3.org/2000/svg" width="'+Math.ceil(vw)+'" height="'+Math.ceil(vh)+'" viewBox="'+vx+' '+vy+' '+vw+' '+vh+'">';
    if (r.defs) svg += '<defs>'+r.defs+'</defs>';
    svg += r.elems.join('');
    svg += '</svg>';
    ...
    if (mode === 1) { // SVG - output SVG XML directly
        r.out = toBytes(svg);
        return r.out.length;
    } else if (mode === 0) { // PNG - rasterize SVG
    ...
    : _width(0), _height(0), _mode(0), _fontSize(12), _fontBold(false), _fontItalic(false), _curX(0), _curY(0)
{
    ...
    _matrix[0]=1; _matrix[1]=0; _matrix[2]=0; _matrix[3]=1; _matrix[4]=0; _matrix[5]=0;
}

JSRenderBackend::~JSRenderBackend() { js_rb_destroy(); }
...
void JSRenderBackend::closeSurface(int mode, bool discard, void* output) {
    if (discard) { js_rb_destroy(); return; }
...
// Path
void JSRenderBackend::beginPath() { js_rb_beginPath(); }
void JSRenderBackend::closePath() { js_rb_closePath(); }
void JSRenderBackend::moveTo(float x, float y) { _curX=x; _curY=y; js_rb_moveTo(x,y); }
void JSRenderBackend::lineTo(float x, float y) { _curX=x; _curY=y; js_rb_lineTo(x,y); }
void JSRenderBackend::relMoveTo(float dx, float dy) { _curX+=dx; _curY+=dy; js_rb_moveTo(_curX,_curY); }
void JSRenderBackend::relLineTo(float dx, float dy) { _curX+=dx; _curY+=dy; js_rb_lineTo(_curX,_curY); }
void JSRenderBackend::curveTo(float x1,float y1,float x2,float y2,float x3,float y3) {
    _curX=x3; _curY=y3; js_rb_curveTo(x1,y1,x2,y2,x3,y3);
}
void JSRenderBackend::arc(float cx,float cy,float r,float a0,float a1) { js_rb_arc(cx,cy,r,a0,a1,0); }
void JSRenderBackend::arcNegative(float cx,float cy,float r,float a0,float a1) { js_rb_arc(cx,cy,r,a0,a1,1); }
void JSRenderBackend::rect(float x,float y,float w,float h) { js_rb_rect(x,y,w,h); }

// Drawing
void JSRenderBackend::fill() { js_rb_fill(); }
void JSRenderBackend::stroke() { js_rb_stroke(); }
void JSRenderBackend::paint() { js_rb_paint(); }

// Style
void JSRenderBackend::setSourceRGB(float r,float g,float b) { js_rb_setColor(r,g,b,1); }
void JSRenderBackend::setSourceRGBA(float r,float g,float b,float a) { js_rb_setColor(r,g,b,a); }
void JSRenderBackend::setLineWidth(float w) { js_rb_setLineWidth(w); }
void JSRenderBackend::setLineJoin(int j) { js_rb_setLineJoin(j); }
void JSRenderBackend::setDash(const double* p,int n,double off) { js_rb_setDash(p,n,off); }
void JSRenderBackend::setOperator(int) {} // SVG doesn't need composite ops
void JSRenderBackend::setAntialias(int) {} // SVG always anti-aliases

// Transform
void JSRenderBackend::save() { js_rb_save(); }
void JSRenderBackend::restore() { js_rb_restore(); }
void JSRenderBackend::translate(float dx,float dy) { js_rb_mmul(1,0,0,1,dx,dy); }
void JSRenderBackend::scale(float sx,float sy) { js_rb_mmul(sx,0,0,sy,0,0); }
void JSRenderBackend::rotate(float a) { js_rb_mmul(cos(a),sin(a),-sin(a),cos(a),0,0); }
void JSRenderBackend::getMatrix(double m[6]) { js_rb_getMatrix(m); }
void JSRenderBackend::setMatrix(const double m[6]) { js_rb_setMatrix(m[0],m[1],m[2],m[3],m[4],m[5]); }
void JSRenderBackend::initIdentityMatrix(double m[6]) { m[0]=1;m[1]=0;m[2]=0;m[3]=1;m[4]=0;m[5]=0; }

void JSRenderBackend::userToDevice(double& x, double& y) {
    double m[6]; js_rb_getMatrix(m);
...
void JSRenderBackend::strokeExtents(double& x1,double& y1,double& x2,double& y2) {
    x1=0; y1=0; x2=_width; y2=_height;
}
...
void JSRenderBackend::selectFontFace(const char* family, bool italic, bool bold) {
    _fontItalic=italic; _fontBold=bold;
...
// Gradient
void JSRenderBackend::setLinearGradient(float x0,float y0,float x1,float y1,
    float r1,float g1,float b1,float r2,float g2,float b2) {
    js_rb_setGradient(x0,y0,x1,y1,r1,g1,b1,r2,g2,b2);
}
void JSRenderBackend::clearPattern() { js_rb_setColor(0,0,0,1); }
...
bool JSRenderBackend::isPathEmpty() { return js_rb_isPathEmpty(); }
