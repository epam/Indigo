...
EmbeddingEnumerator::EmbeddingEnumerator(Graph& supergraph)
    : CP_INIT, TL_CP_GET(_core_1), TL_CP_GET(_core_2), TL_CP_GET(_term2), TL_CP_GET(_unterm2), TL_CP_GET(_mapped_orbit_ids), TL_CP_GET(_g1_fast),
      TL_CP_GET(_g2_fast),
...
EmbeddingEnumerator::_Enumerator::_Enumerator(EmbeddingEnumerator& context) : _context(context), _mapped_orbit_ids(context._mapped_orbit_ids)
{
    _t1_len = 0;
    _t2_len = 0;
    _core_len = 0;
...
EmbeddingEnumerator::_Enumerator::_Enumerator(const EmbeddingEnumerator::_Enumerator& other)
    : _context(other._context), _mapped_orbit_ids(other._context._mapped_orbit_ids)
{
...
bool EmbeddingEnumerator::_Enumerator::_checkPair(int node1, int node2)
{
    ...
    if (_use_equivalence)
    {
        ...
        if (eq_class != -1)
        {
            ...
            auto it = _mapped_orbit_ids.find(pair_id);
            if ( it != _mapped_orbit_ids.end() )
            {
                if (needRemove && _context.cb_vertex_remove != 0)
                    _context.cb_vertex_remove(*_context._g1, node1, _context.userdata);
                return false;
            }
            _mapped_orbit_ids.emplace(pair_id);
        }
    }
