...
{
    QS_DEF(ArrayInt, vert_mapping);

    QS_DEF(ObjArray<ArrayInt>, subgraph_cycles);
        ...
        for (int i = c_vertex.neiBegin(); i < c_vertex.neiEnd(); i = c_vertex.neiNext(i))
        {
            ...
            if (!visited_edges.find(edge))
            {
                ...
                else
                {
                    ...
                    ArrayInt& edges_of_cycle = subgraph_cycles.push();
    ...
    {
        ArrayInt& cycle_edges = subgraph_cycles[i];
        ArrayInt& new_cycle_edges = _cycles.push();
...
void SimpleCycleBasis::_minimize(int startIndex)
{
    ...
    for (int cur_cycle = startIndex; cur_cycle < _cycles.size(); ++cur_cycle)
    {
        ...
        QS_DEF(ObjArray<ArrayInt>, all_new_cycles);
        ...
        for (int v = _graph.vertexBegin(); v < _graph.vertexEnd(); v = _graph.vertexNext(v))
        {
            ...
            if (shouldSearchCycle)
            {
                ...
                ArrayInt& edges_of_new_cycle = all_new_cycles.push();
                ArrayInt path_vertices;
        ...
        ArrayInt& current_cycle = _cycles.at(cur_cycle);
        ...
        {
            current_cycle.clear();
            ArrayInt& sh_cycle = all_new_cycles[shortest_cycle];
...
void SimpleCycleBasis::_prepareSubgraph(Graph& subgraph)
{
    ...
    while (remaining_edges.size() > 0)
    {
        ...
        ArrayInt& path_edges = _cycles.push();
