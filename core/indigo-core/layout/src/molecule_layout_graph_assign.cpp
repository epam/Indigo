...
void MoleculeLayoutGraph::_findFixedComponents(BiconnectedDecomposer& bc_decom, ArrayInt& fixed_components, PtrArray<MoleculeLayoutGraph>& bc_components)
{
    // 1. Find biconnected components forming connected subgraph from fixed vertices
    if (_n_fixed == 0)
        return;
    ...
    Graph fixed_graph;
    QS_DEF(ArrayInt, fixed_mapping);
    QS_DEF(ArrayInt, fixed_inv_mapping);
    ...
    else
    {
        ...
        const ArrayInt& decomposition = fixed_graph.getDecomposition();
...
bool MoleculeLayoutGraph::_assignComponentsRelativeCoordinates(PtrArray<MoleculeLayoutGraph>& bc_components, ArrayInt& fixed_components,
...
void MoleculeLayoutGraph::_findFirstVertexIdx(int n_comp, ArrayInt& fixed_components, PtrArray<MoleculeLayoutGraph>& bc_components, bool all_trivial)
{
    if (_n_fixed > 0)
    {
        int j = -1;
...
bool MoleculeLayoutGraph::_prepareAssignedList(ArrayInt& assigned_list, BiconnectedDecomposer& bc_decom, PtrArray<MoleculeLayoutGraph>& bc_components,
                                               ArrayInt& bc_tree)
{
    assigned_list.clear();
