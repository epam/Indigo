...
bool ReactionSubstructureMatcher::_checkAAM()
{
    ...
    QS_DEF(ObjArray<std::set<int>>, classes_mapping_left);
    QS_DEF(ObjArray<std::set<int>>, classes_mapping_right);
    int i;
    ...
    for (i = 0; i < classes_mapping_right.size(); i++)
    {
        ...
        std::set<int>& right = classes_mapping_right[i];
        std::set<int>& left = classes_mapping_left[i];
