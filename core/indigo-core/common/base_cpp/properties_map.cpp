...
void PropertiesMap::copy(RedBlackStringObjMap<ArrayChar>& other)
{
    clear();
...
ArrayChar& PropertiesMap::insert(const char* key)
{
    insert(key, 0);
    return valueBuf(key);
}
...
ArrayChar& PropertiesMap::valueBuf(const char* key)
{
