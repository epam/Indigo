...
namespace indigo
{
    ...
    public:
        ...
        ArrayNew<int> possible_cis_trans_to_check;
    ...
    protected:
        ...
        static bool _check_automorphism(Graph& graph, const ArrayNew<int>& mapping, const void* context);
        ...
        static int _compare_mapped(Graph& graph, const ArrayNew<int>& mapping1, const ArrayNew<int>& mapping2, const void* context);
        ...
        int _compareMappedStereocenters(Molecule& mol, const ArrayNew<int>& mapping1, const ArrayNew<int>& mapping2, const ArrayNew<int>& inv_mapping1,
                                        const ArrayNew<int>& inv_mapping2) const;

        bool _checkStereocentersAutomorphism(Molecule& mol, const ArrayNew<int>& mapping) const;

        void _initialize(Molecule& mol);
        void _calculateHydrogensAndDegree(Molecule& mol);
        void _getFirstApproximation(Molecule& mol);

        int _validCisTransBond(int idx, const ArrayNew<int>& orbits);
        int _validStereocenter(int idx, ArrayNew<int>& orbits, int* parity = 0);
        int _validStereocenterByAtom(int atom_idx, ArrayNew<int>& orbits, int* parity = 0);
        ...
        void _markValidOrInvalidStereo(bool find_valid, ArrayNew<int>& approximation_orbits, bool* found);
        ...
        static void _getSortedNei(Graph& g, int v, Array<EdgeInfo>& sorted_nei, ArrayNew<int>& inv_mapping);
        int _getMappedBondOrderAndParity(Molecule& m, int e, ArrayNew<int>& inv_mapping) const;

        TL_CP_DECL(ArrayNew<int>, _approximation_orbits);
        TL_CP_DECL(ArrayNew<int>, _approximation_orbits_saved);
        TL_CP_DECL(ArrayNew<int>, _hcount);
        TL_CP_DECL(ArrayNew<int>, _cistrans_stereo_bond_parity);
        TL_CP_DECL(ArrayNew<int>, _degree);
        TL_CP_DECL(ArrayNew<int>, _independent_component_index);
        ...
        TL_CP_DECL(ArrayNew<int>, _stereocenter_state);
        TL_CP_DECL(ArrayNew<int>, _cistrans_bond_state);
