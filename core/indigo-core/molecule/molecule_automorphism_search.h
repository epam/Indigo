...
namespace indigo
{
    ...
    public:
        ...
        ArrayInt possible_cis_trans_to_check;
    ...
    protected:
        ...
        static bool _check_automorphism(Graph& graph, const ArrayInt& mapping, const void* context);
        ...
        static int _compare_mapped(Graph& graph, const ArrayInt& mapping1, const ArrayInt& mapping2, const void* context);
        ...
        int _compareMappedStereocenters(Molecule& mol, const ArrayInt& mapping1, const ArrayInt& mapping2, const ArrayInt& inv_mapping1,
                                        const ArrayInt& inv_mapping2) const;

        bool _checkStereocentersAutomorphism(Molecule& mol, const ArrayInt& mapping) const;

        void _initialize(Molecule& mol);
        void _calculateHydrogensAndDegree(Molecule& mol);
        void _getFirstApproximation(Molecule& mol);

        int _validCisTransBond(int idx, const ArrayInt& orbits);
        int _validStereocenter(int idx, ArrayInt& orbits, int* parity = 0);
        int _validStereocenterByAtom(int atom_idx, ArrayInt& orbits, int* parity = 0);
        ...
        void _markValidOrInvalidStereo(bool find_valid, ArrayInt& approximation_orbits, bool* found);
        ...
        static void _getSortedNei(Graph& g, int v, Array<EdgeInfo>& sorted_nei, ArrayInt& inv_mapping);
        int _getMappedBondOrderAndParity(Molecule& m, int e, ArrayInt& inv_mapping) const;

        TL_CP_DECL(ArrayInt, _approximation_orbits);
        TL_CP_DECL(ArrayInt, _approximation_orbits_saved);
        TL_CP_DECL(ArrayInt, _hcount);
        TL_CP_DECL(ArrayInt, _cistrans_stereo_bond_parity);
        TL_CP_DECL(ArrayInt, _degree);
        TL_CP_DECL(ArrayInt, _independent_component_index);
        ...
        TL_CP_DECL(ArrayInt, _stereocenter_state);
        TL_CP_DECL(ArrayInt, _cistrans_bond_state);
