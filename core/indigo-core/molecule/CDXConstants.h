...
enum CDXDatumID
{
    ...
    kCDXProp_CreationProgram,      // 0x0003 The name of the program, including version and platform, that created the associated CDX object. ChemDraw 4.0 uses
                                   // ChemDraw 4.0 as the value of CreationProgram. (CDXString)
    ...
    kCDXProp_ModificationProgram, // 0x0006 The name of the program, including version and platform, of the last program to perform a modification. ChemDraw 4.0
                                  // uses ChemDraw 4.0 as the value of CreationProgram. (CDXString)
    ...
    kCDXProp_RegistryAuthority, // 0x000C A string that specifies the authority which issued a registry or catalog number. Some examples of registry authorities
    ...
    kCDXProp_3DPosition, // 0x0201 The 3D location (in the order of X-, Y-, and Z-locations in right-handed coordinate system) of an object in CDX coordinate
                         // units. The precise meaning of this attribute varies depending on the type of object. (CDXPoint3D)
    kCDXProp_2DExtent,   // 0x0202 The width and height of an object in CDX coordinate units. The precise meaning of this attribute varies depending on the type
                         // of object. (CDXPoint2D)
    kCDXProp_3DExtent, // 0x0203 The width, height, and depth of an object in CDX coordinate units (right-handed coordinate system). The precise meaning of this
    ...
    kCDXProp_3DHead,      // 0x0207 The 3D location (in the order of X-, Y-, and Z-locations in right-handed coordinate system) of the head of an object in CDX
                          // coordinate units. The precise meaning of this attribute varies depending on the type of object. (CDXPoint3D)
    kCDXProp_3DTail,      // 0x0208 The 3D location (in the order of X-, Y-, and Z-locations in right-handed coordinate system) of the tail of an object in CDX
                          // coordinate units. The precise meaning of this attribute varies depending on the type of object. (CDXPoint3D)
    ...
    kCDXProp_Atom_Formula, // 0x0404 The composition of a node representing a fragment whose composition is known, but whose connectivity is not. For example,
                           // C<sub>4</sub>H<sub>9</sub> represents a mixture of the 4 butyl isomers. (CDXFormula)
    ...
    kCDXProp_Atom_NumHydrogens, // 0x042B The number of (explicit) hydrogens in a labeled atom consisting of one heavy atom and (optionally) the symbol H (e.g.,
                                // CH<sub>3</sub>). (UINT16)
    kCDXProp_Unused4,           // 0x042C
    kCDXProp_Unused5,           // 0x042D
    kCDXProp_Atom_HDot,  // 0x042E Signifies the presence of an implicit hydrogen with stereochemistry specified equivalent to an explicit H atom with a wedged
                         // bond. (CDXBooleanImplied)
    kCDXProp_Atom_HDash, // 0x042F Signifies the presence of an implicit hydrogen with stereochemistry specified equivalent to an explicit H atom with a hashed
                         // bond. (CDXBooleanImplied)
    kCDXProp_Atom_Geometry,     // 0x0430 The geometry of the bonds about this atom. (INT8)
    kCDXProp_Atom_BondOrdering, // 0x0431 An ordering of the bonds to this node, used for stereocenters, fragments, and named alternative groups with more than
                                // one attachment. (CDXObjectIDArray)
    kCDXProp_Node_Attachments,  // 0x0432 For multicenter attachment nodes or variable attachment nodes, a list of IDs of the nodes which are multiply or
                                // variably attached to this node. (CDXObjectIDArrayWithCounts)
    ...
    kCDXProp_LineStarts, // 0x0704 The number of lines of a text object followed by that many values indicating the zero-based text position of each line start.
                         // (INT16ListWithCounts)
    ...
    // Document properties.
    kCDXProp_MacPrintInfo = 0x0800, // 0x0800 The 120 byte Macintosh TPrint data associated with the CDX document object. Refer to Macintosh Toolbox manual for
    ...
    kCDXProp_Picture_Edition =
        0x0A60, // 0x0A60 The section information (SectionHandle) of the Macintosh Publish & Subscribe edition embedded in the CDX picture object. (Unformatted)
    kCDXProp_Picture_EditionAlias, // 0x0A61 The alias information of the Macintosh Publish & Subscribe edition embedded in the CDX picture object.
                                   // (Unformatted)
    ...
    kCDXProp_TLC_OriginFraction =
        0x0AA0, // 0x0AA0 The distance of the origin line from the bottom of a TLC Plate, as a fraction of the total height of the plate. (FLOAT64)
    kCDXProp_TLC_SolventFrontFraction, // 0x0AA1 The distance of the solvent front from the top of a TLC Plate, as a fraction of the total height of the plate.
                                       // (FLOAT64)
    ...
    kCDXProp_NamedAlternativeGroup_GroupFrame, // 0x0B01 The bounding box of the lower portion of the Named Alternative Group, containing the definition of the
                                               // group. (CDXRectangle)
    ...
    kCDXProp_ReactionStep_Atom_Map_Manual,   // 0x0C07 Represents pairs of mapped atom IDs; each pair is a reactant atom mapped to to a product atom.
                                             // (CDXObjectIDArray)
    ...
    kCDXProp_CrossReference_Container =
        0x0F00, // 0x0F00 An external object containing (as an embedded object) the document containing the Sequence object being referenced. (CDXString)
