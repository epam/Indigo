...
void BaseMolecule::collapse(BaseMolecule& bm, int id, Mapping& mapAtom, Mapping& mapBondInv)
{
    ...
    for (int j = 0; j < group.atoms.size(); ++j)
    {
        ...
        if (mapAtom.find(from) == mapAtom.end())
            mapAtom.insert({from, to});
        else if (mapAtom.at(from) != to)
...
{
    if (_stereo_flag_positions.find(frag_index) != _stereo_flag_positions.end())
    {
        pos = _stereo_flag_positions.at(frag_index);
        return true;
    }
    return false;
}
