...
void MainLayerFormula::printFormula(ArrayChar& result)
{
    ...
    const ArrayInt& atom_lables_sorted = MoleculeInChIUtils::getLexSortedAtomLables();
...
int MainLayerConnections::compareMappings(const MoleculeInChIUtils::Mapping& m1, const MoleculeInChIUtils::Mapping& m2)
{
    ...
    QS_DEF(ArrayInt, tmp1);
    QS_DEF(ArrayInt, tmp2);
...
void MainLayerConnections::printConnectionTable(ArrayChar& result)
{
    ...
    QS_DEF(ArrayInt, descedants_size);
    ...
    QS_DEF(ArrayInt, vertex_visited);
    vertex_visited.clear_resize(cano_mol.vertexEnd());
    vertex_visited.zerofill();

    QS_DEF(ArrayInt, vertex_stack);
    vertex_stack.push(min_degree_vertex);

    QS_DEF(ArrayInt, nei_visited_vertices);
    QS_DEF(ArrayInt, nei_dfs_next_vertices);
...
void HydrogensLayer::print(ArrayChar& result)
{
    ...
    const ArrayInt& hydrogens = _per_atom_immobile;
    int max_hydrogens = 0;
...
bool HydrogensLayer::checkAutomorphism(const ArrayInt& mapping)
{
    Molecule& mol = _getMolecule();
...
bool CisTransStereochemistryLayer::checkAutomorphism(const ArrayInt& mapping)
{
    Molecule& mol = _getMolecule();
...
bool TetrahedralStereochemistryLayer::checkAutomorphism(const ArrayInt& mapping)
{
    Molecule& mol = _getMolecule();
