...
void MoleculePiSystemsMatcher::_calcConnectivity(Molecule& mol, ArrayInt& conn)
{
...
void MoleculePiSystemsMatcher::_calculatePiSystemsSizes(int n_pi_systems, ArrayInt& sizes)
{
...
{
    QS_DEF(ArrayInt, pi_system_size);
...
{
    const ArrayInt& pi_system_per_vertex = _decomposer->getDecomposition();
    for (int v = _target.vertexBegin(); v != _target.vertexEnd(); v = _target.vertexNext(v))
    {
