            ...
            for (int i = _bmol->vertexBegin(); i != _bmol->vertexEnd(); i = _bmol->vertexNext(i))
            {
                ...
                if (label.size())
                {
                    ...
                    if (label.size() > 2 && strncmp(label.c_str(), "_R", 2) == 0 && sscanf(label.c_str() + 2, "%d", &rnum) == 1)
                    {
                        ...
                        BufferScanner strscan(label.c_str());
                        QS_DEF(std::string, word);
                        while (!strscan.isEOF())
                        {
                            strscan.skip(1);
                            strscan.readWord(word, ",;");
                            if (word.size() >= 3 && strncmp(word.c_str(), "_R", 2) == 0 && sscanf(word.c_str() + 2, "%d", &rnum) == 1)
                                _bmol->allowRGroupOnRSite(i, rnum);
                        }
                    }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                QueryMolecule::Atom* atom = _qmol->releaseAtom(i);
                                atom->removeConstraints(QueryMolecule::ATOM_NUMBER);
                                _qmol->resetAtom(i, QueryMolecule::Atom::und(atom, new QueryMolecule::Atom(QueryMolecule::ATOM_PSEUDO, label.c_str())));
                            }
                        }
                    }
                }
        ...
        if (!first_atom)
        {
            ...
            while (isdigit(next) || next == '%')
            {
                ...
                else if (_cycles[number].pending_bond >= 0)
                {
                    ...
                    if (_qmol != 0)
                    {
                        QS_DEF(std::string, bond_str);
                        AutoPtr<QueryMolecule::Bond> qbond(new QueryMolecule::Bond());

                        bond_str = _pending_bonds_pool.at(_cycles[number].pending_bond_str);
                ...
                if (isdigit(next) || next == '%')
                {
                    ...
                    else if (number >= 0 && number < _cycles.size() && _cycles[number].pending_bond >= 0)
                    {
                        ...
                        if (bond_str.size() )
                        {
                            if (!ignore_closing_bond_direction_mismatch)
                            {
                                if ((int)strlen(str) != bond_str.size() || memcmp(str, bond_str.c_str(), strlen(str)) != 0)
...
void SmilesLoader::_readBond(std::string& bond_str, _BondDesc& bond, AutoPtr<QueryMolecule::Bond>& qbond)
{
    if (bond_str.find(';') != -1)
    {
        QS_DEF(std::string, substring);
...
void SmilesLoader::_readBondSub(std::string& bond_str, _BondDesc& bond, AutoPtr<QueryMolecule::Bond>& qbond)
{
...
void SmilesLoader::_readAtom(std::string& atom_str, bool first_in_brackets, _AtomDesc& atom, AutoPtr<QueryMolecule::Atom>& qatom)
{
    ...
    while (!scanner.isEOF())
    {
        ...
        else if (next == '$')
        {
            ...
            fragment->fragment_smarts = subexp;
...
int SmilesLoader::_parseCurly(std::string& curly, int& repetitions)
{
    ...
    if (curly.size() >= 2 && curly[0] == '+')
    {
        ...
        {
            repetitions = 0;
            BufferScanner scanner(curly.c_str() + 2, curly.size() - 2);
