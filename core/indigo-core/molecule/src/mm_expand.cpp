...
void placeRingMonomers(KetDocument& mol, const std::vector<float>& dimensions,
                              Graph& graph, std::unordered_map<std::string, Vec2f>& newPositions,
                              std::unordered_set<std::string>& placed)
{
    ...
    for (int ci = 0; ci < ringCount; ++ci) {
        ...
        center.x/= cycleSize; center.y/= cycleSize;
        for (size_t i = 0; i < cycleSize; ++i) {
            float ang = i*phi;
...
void bfsPropagate(KetDocument& mol, const std::vector<float>& dimensions,
                         const std::unordered_map<std::string, std::vector<NeighborSpec>>& adjacency,
                         std::unordered_map<std::string, Vec2f>& newPos,
                         std::unordered_set<std::string>& placed)
{
...
float evalLeavingDistance(const Vec2f& pBase, const Vec2f& p0, const Vec2f& p1,
                                 ...
                                 float rot)
{
    ...
    Vec2f d0{a0.x - cen0.x, a0.y - cen0.y}; Vec2f d1{cen1.x - a1.x, cen1.y - a1.y};
...
static void computeRotations(KetDocument& mol,
                             const std::unordered_map<std::string, std::vector<NeighborSpec>>& neighborMap,
...
void applyTransformations(KetDocument& mol,
                                 const std::unordered_map<std::string, Vec2f>& newPos,
                                 const std::unordered_map<std::string, float>& newAngles)
{
    for (const auto& id : mol.monomersIds()) {
        auto& m = static_cast<KetMonomer&>(*mol.getMonomerById(id));
        // determine rotation
