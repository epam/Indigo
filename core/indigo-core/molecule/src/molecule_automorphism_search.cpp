...
bool MoleculeAutomorphismSearch::_isStereocenterMappedRigid(Molecule& baseMolecule, int i, const int* mapping)
{
    int idx, type, group;
    int pyramid[4];
    int j;

    baseMolecule.stereocenters.get(i, idx, type, group, pyramid);
    ...
    if (baseMolecule.stereocenters.getType(mapping[idx]) != type)
        throw Error("internal: stereocenter types mismatch");

    int pyra_map[4];

    MoleculeStereocenters::getPyramidMapping(baseMolecule, baseMolecule, idx, mapping, pyra_map, false);
