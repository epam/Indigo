...
void MoleculeFingerprintBuilder::_makeFingerprint(BaseMolecule& mol)
{
    ...
    {
        _tau_super_structure = std::make_unique<TautomerSuperStructure>(mol.asMolecule());
        mol_for_enumeration = _tau_super_structure.get();
    }
    else
        _tau_super_structure.reset(nullptr);
