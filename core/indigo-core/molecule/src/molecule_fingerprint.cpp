...
{
    std::shared_ptr<BaseMolecule> mol_for_enumeration(&mol);

    if (!query && _parameters.tau_qwords > 0 && !skip_tau)
    {
        std::unique_ptr<TautomerSuperStructure> tau_super_structure = std::make_unique<TautomerSuperStructure>(mol.asMolecule());

        _tau_super_structure = std::move(tau_super_structure);
