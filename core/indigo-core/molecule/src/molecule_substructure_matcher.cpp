...
int MoleculeSubstructureMatcher::_embedding_markush(int* core_sub, int* core_super)
{
    ...
    // do not do this:
    // const ArrayInt &rg_list = g1.getRGroups()->getSiteRGroups(old_site_idx);

    QS_DEF(ArrayInt, old_site_rgroups);
...
{
    QS_DEF(ArrayInt, occurrences);
    QS_DEF(ArrayInt, conditions);
    QS_DEF(ArrayInt, nontop_rgroups);
...
void MoleculeSubstructureMatcher::makeTransposition(BaseMolecule& mol, ArrayInt& transposition_out)
{
    int i;
