...
void CmfSaver::_encodeUIntArray(const ArrayInt& data, const ArrayInt& mapping)
{
...
void CmfSaver::_encodeUIntArray(const ArrayInt& data)
{
    _output->writePackedUInt(data.size());
    for (int i = 0; i < data.size(); i++)
    {
...
void CmfSaver::_encodeUIntArraySkipNegative(const ArrayInt& data)
{
    int len = 0;
    for (int i = 0; i < data.size(); i++)
        if (data[i] >= 0)
...
const ArrayInt& CmfSaver::getAtomSequence()
{
