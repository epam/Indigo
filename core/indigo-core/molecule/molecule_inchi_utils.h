...
namespace indigo
{
    ...
    public:
        ...
        struct Mapping
        {
            Mapping(const ArrayInt& _mapping, const ArrayInt& _inv_mapping) : mapping(_mapping), inv_mapping(_inv_mapping)
            {
            }

            const ArrayInt&mapping, &inv_mapping;
        };

        // Returns indices for lexicographically-sorted atom labels
        // with exception that the first atom is Carbon
        static const ArrayInt& getLexSortedAtomLables();
        // Returns inverse permutation for getLexSortedLables
        static const ArrayInt& getLexSortedAtomLablesRanks();
        ...
        static void stableSmallSort(ArrayInt& indices, const ArrayInt* ranks);
