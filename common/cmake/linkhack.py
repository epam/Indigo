...
def linux(compiler, linkFlags, objFiles, linkLibraries, target):
    ...
    if target.find('libindigo.so') != -1 or target.find('bingo_postgres.so') != -1 or target.find('libbingo-oracle.so') != -1:
...
def mac(compiler, linkFlags, objFiles, linkLibraries, target):
    ...
    if target.find('libindigo.dylib') != -1 or target.find('bingo_postgres.dylib') != -1:
