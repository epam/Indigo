...
namespace indigo
{
    ...
    public:
        explicit VectorString(const std::vector<std::string>& v) : vec(v)
        {
        }
        friend std::stringstream& operator<<(std::stringstream& ss, const VectorString& vs);
    };

    std::stringstream& operator<<(std::stringstream& ss, const VectorString& vs)
    {
        bool isFirst = true;
                            ...
                            const std::vector<int>& selected_atoms)
    {
        ...
        molecularWeightStream << VectorString(molWeights);
        mostAbundantMassStream << VectorString(mamMasses);
        monoisotopicMassStream << VectorString(misoMasses);
        massCompositionStream << VectorString(massCompositions);
        grossFormulaStream << VectorString(grossFormulas);
    }
