...
import platform
import sys
from ctypes import CDLL, POINTER, c_char_p, c_float, c_int, pointer
...
class Bingo(object):
    def __init__(self, bingoId, indigo, lib):
        ...
        self._lib.bingoCreateDatabaseFile.argtypes = [
            c_char_p,
            c_char_p,
            c_char_p,
        ]
        ...
        self._lib.bingoInsertRecordObjWithExtFP.argtypes = [
            c_int,
            c_int,
            c_int,
        ]
        ...
        self._lib.bingoInsertRecordObjWithIdAndExtFP.argtypes = [
            c_int,
            c_int,
            c_int,
            c_int,
        ...
        self._lib.bingoSearchSim.argtypes = [
            c_int,
            c_int,
            c_float,
            c_float,
        ...
        self._lib.bingoSearchSimTopN.argtypes = [
            c_int,
            c_int,
            c_int,
            c_float,
        ...
        self._lib.bingoEstimateRemainingTime.argtypes = [
            c_int,
            POINTER(c_float),
        ]
    ...
    def close(self):
        if self._id >= 0:
            self._indigo._setSessionId()
            Bingo._checkResult(
                self._indigo, self._lib.bingoCloseDatabase(self._id)
            )
            self._id = -1
    ...
    def loadDatabaseFile(indigo, path, options=""):
        ...
        return Bingo(
            Bingo._checkResult(
                indigo,
                lib.bingoLoadDatabaseFile(
                    path.encode("ascii"), options.encode("ascii")
                ),
            ),
            indigo,
            lib,
    ...
    def delete(self, index):
        self._indigo._setSessionId()
        Bingo._checkResult(
            self._indigo, self._lib.bingoDeleteRecord(self._id, index)
        )
        ...
        return BingoObject(
            Bingo._checkResult(
                ...
                self._lib.bingoSearchSub(
                    self._id, query.id, options.encode("ascii")
                ),
            ),
            self._indigo,
            self,
    ...
    def getRecordById(self, id):
        ...
        return IndigoObject(
            self._indigo,
            Bingo._checkResult(
                self._indigo, self._lib.bingoGetRecordObj(self._id, id)
            ),
        )
...
class BingoObject(object):
    ...
    def close(self):
        ...
        if self._id >= 0:
            Bingo._checkResult(
                self._indigo, self._bingo._lib.bingoEndSearch(self._id)
            )
            self._id = -1
    ...
    def next(self):
        ...
        return (
            Bingo._checkResult(
                self._indigo, self._bingo._lib.bingoNext(self._id)
            )
            == 1
        )
    ...
    def getCurrentId(self):
        self._indigo._setSessionId()
        return Bingo._checkResult(
            self._indigo, self._bingo._lib.bingoGetCurrentId(self._id)
        )
    ...
    def getIndigoObject(self):
        ...
        return IndigoObject(
            self._indigo,
            Bingo._checkResult(
                self._indigo, self._bingo._lib.bingoGetObject(self._id)
            ),
        )
    ...
    def getCurrentSimilarityValue(self):
        ...
        return Bingo._checkResult(
            self._indigo,
            self._bingo._lib.bingoGetCurrentSimilarityValue(self._id),
        )
    ...
    def estimateRemainingResultsCount(self):
        ...
        return Bingo._checkResult(
            self._indigo,
            self._bingo._lib.bingoEstimateRemainingResultsCount(self._id),
        )
    ...
    def estimateRemainingResultsCountError(self):
        ...
        return Bingo._checkResult(
            self._indigo,
            self._bingo._lib.bingoEstimateRemainingResultsCountError(self._id),
        )
    ...
    def estimateRemainingTime(self):
        ...
        Bingo._checkResult(
            self._indigo,
            self._bingo._lib.bingoEstimateRemainingTime(
                self._id, pointer(value)
            ),
        )
        return value.value
    ...
    def containersCount(self):
        self._indigo._setSessionId()
        return Bingo._checkResult(
            self._indigo, self._bingo._lib.bingoContainersCount(self._id)
        )

    def cellsCount(self):
        self._indigo._setSessionId()
        return Bingo._checkResult(
            self._indigo, self._bingo._lib.bingoCellsCount(self._id)
        )

    def currentCell(self):
        self._indigo._setSessionId()
        return Bingo._checkResult(
            self._indigo, self._bingo._lib.bingoCurrentCell(self._id)
        )

    def minCell(self):
        self._indigo._setSessionId()
        return Bingo._checkResult(
            self._indigo, self._bingo._lib.bingoMinCell(self._id)
        )

    def maxCell(self):
        self._indigo._setSessionId()
        return Bingo._checkResult(
            self._indigo, self._bingo._lib.bingoMaxCell(self._id)
        )
