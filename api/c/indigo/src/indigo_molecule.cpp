...
IndigoDataSGroupsIter::IndigoDataSGroupsIter(BaseMolecule& molecule, ArrayInt& refs) : IndigoObject(DATA_SGROUPS_ITER), _mol(molecule), _refs(refs)
{
    _idx = -1;
}
...
IndigoSuperatomsIter::IndigoSuperatomsIter(BaseMolecule& molecule, ArrayInt& refs) : IndigoObject(SUPERATOMS_ITER), _mol(molecule), _refs(refs)
{
    _idx = -1;
}
...
IndigoRepeatingUnitsIter::IndigoRepeatingUnitsIter(BaseMolecule& molecule, ArrayInt& refs) : IndigoObject(REPEATING_UNITS_ITER), _mol(molecule), _refs(refs)
{
    _idx = -1;
}
...
IndigoMultipleGroupsIter::IndigoMultipleGroupsIter(BaseMolecule& molecule, ArrayInt& refs) : IndigoObject(MULTIPLE_GROUPS_ITER), _mol(molecule), _refs(refs)
{
    _idx = -1;
}
...
IndigoGenericSGroupsIter::IndigoGenericSGroupsIter(BaseMolecule& molecule, ArrayInt& refs) : IndigoObject(GENERIC_SGROUPS_ITER), _mol(molecule), _refs(refs)
{
    _idx = -1;
}
...
IndigoSGroupsIter::IndigoSGroupsIter(BaseMolecule& molecule, ArrayInt& sg_refs) : IndigoObject(SGROUPS_ITER), _mol(molecule), _refs(sg_refs)
{
    _idx = -1;
}
...
static void _parseRSites(const char* name, ArrayInt& rsites)
{
...
IndigoSubmolecule::IndigoSubmolecule(BaseMolecule& mol_, ArrayInt& vertices_, ArrayInt& edges_) : IndigoObject(SUBMOLECULE), _mol(mol_)
{
...
void IndigoSubtreesIter::_handleTree(Graph& graph, const ArrayInt& vertices, const ArrayInt& edges, void* context)
{
    IndigoSubtreesIter* self = (IndigoSubtreesIter*)context;

    ArrayInt& self_vertices = self->_vertices.push();
    ArrayInt& self_edges = self->_edges.push();
...
bool IndigoRingsIter::_handleCycle(Graph& graph, const ArrayInt& vertices, const ArrayInt& edges, void* context)
{
...
void IndigoEdgeSubmoleculeIter::_handleSubgraph(Graph& graph, const int* v_mapping, const int* e_mapping, void* context)
{
    ...
    ArrayInt& vertices = self->_vertices.push();
    ArrayInt& edges = self->_edges.push();
