...
int IndigoDeconvolution::_rGroupsEmbedding(Graph& graph1, Graph& graph2, int* map, int* inv_map, void* userdata)
{
    ...
    QS_DEF(ArrayInt, queue);
    QS_DEF(ArrayInt, queue_markers);
    int n_rgroups = 0;
     ...
     */
    ArrayInt& visited_atoms = deco_match.visitedAtoms;
    /*
     * Each array corresponds to a separate Rgroup.
     * Order - atom number for scaffold
     * Index - atom number for Rgroup
     */
    ObjArray<ArrayInt>& attachment_order = deco_match.attachmentOrder;
    ObjArray<ArrayInt>& attachment_index = deco_match.attachmentIndex;
     ...
     */
    ArrayInt& scaf_bonds = deco_match.scaffoldBonds;
    scaf_bonds.clear();
     ...
     */
    ArrayInt& scaf_atoms = deco_match.scaffoldAtoms;
    scaf_atoms.clear();
...
void IndigoDeconvolution::createRgroups(IndigoDecompositionMatch& deco_match, bool change_scaffold)
{
    ...
    QS_DEF(ArrayInt, inv_scaf_map);
    QS_DEF(ArrayInt, rg_mapping);
    QS_DEF(ArrayInt, rgidx_map);
    ...
    {
        ArrayInt& att_indexes = attachment_index[rg_idx];
        ArrayInt& att_orders = attachment_order[rg_idx];
...
void IndigoDecompositionMatch::renumber(ArrayInt& map, ArrayInt& inv_map)
{
    QS_DEF(ArrayInt, tmp_array);
...
void IndigoDecompositionMatch::copyScafAutoMaps(ObjList<ArrayInt>& autoMaps)
{
    scafAutoMaps.clear();
...
// */
// void IndigoDeconvolution::addCompleteRGroup(IndigoDecompositionMatch& deco_match, bool change_scaffold, ArrayInt* rg_map) {
//   Molecule& mol_set = deco_match.mol_out;
//   ObjArray< ArrayInt >& attachment_order = deco_match.attachmentOrder;
//   ObjArray< ArrayInt >& attachment_index = deco_match.attachmentIndex;
//   ArrayInt& map = deco_match.lastInvMapping;
//
...
//    */
//   QS_DEF(RedBlackStringObjMap< ArrayInt >, match_rgroups);
//   match_rgroups.clear();
//   /*
...
////         ArrayInt& value = match_rgroups.value(i);
...
//      if(match_rgroups.find(str_key.ptr())) {
//         ArrayInt& match_r = match_rgroups.at(str_key.ptr());
//         /*
//          * Remove matches
//          */
//         if(match_r.size() == 0 ) {
...
/*
 */
void IndigoDeconvolution::_addFullRGroup(IndigoDecompositionMatch& deco_match, ArrayInt& auto_map, int rg_idx, int new_rg_idx)
{
    Molecule& qmol = deco_match.mol_out;
    ArrayInt& map = deco_match.lastInvMapping;
    ArrayInt& att_orders = deco_match.attachmentOrder[rg_idx];
    ArrayInt& att_indexes = deco_match.attachmentIndex[rg_idx];
    /*
...
void IndigoDeconvolution::_makeInvertMap(ArrayInt& map, ArrayInt& invmap)
{
    for (int i = 0; i < map.size(); i++)
    {
        if (map[i] != -1)
...
int IndigoDeconvolution::_createRgMap(IndigoDecompositionMatch& deco_match, int aut_idx, RedBlackStringObjMap<ArrayInt>& match_rgroups_in, ArrayInt* rg_map,
                                      bool change_scaffold)
{
    ...
    ArrayInt& auto_map = deco_match.scafAutoMaps[aut_idx];
    /*
     * A set to keep attachment orders
     */
    QS_DEF(RedBlackSet<int>, str_keys);
...
int IndigoDeconvolution::_getRgScore(ArrayInt& rg_map) const
{
    int result = 0;
...
void IndigoDeconvolution::DecompositionEnumerator::calculateAutoMaps(Graph& sub)
{
    ...
    ArrayInt& d_map = _scafAutoMaps.at(l_idx);
...
bool IndigoDeconvolution::DecompositionEnumerator::shouldContinue(int* map, int size)
{
    ...
    {
        ArrayInt& scaf_atoms = contexts[m_idx].scaffoldAtoms;
...
void IndigoDeconvolution::DecompositionEnumerator::addMatch(IndigoDecompositionMatch& match, Graph& sub, Graph& super)
{
    ...
    ObjArray<ArrayInt> rsite_orders;
    RedBlackMap<int, int> r_sites;
    QS_DEF(RedBlackSet<int>, processed_r);
    QS_DEF(ArrayInt, swap_order);
    ArrayInt& direct_order = rsite_orders.push();

    r_molecule.clone_KeepIndices((BaseMolecule&)sub, 0);
    if (remove_rsites)
    {
        QS_DEF(ArrayInt, atoms_r);
        atoms_r.clear();
    ...
    {
        ArrayInt& auto_map = _autoMaps[auto_idx];
        /*
...
bool IndigoDeconvolution::DecompositionEnumerator::_foundOrder(ObjArray<ArrayInt>& rsite_orders, ArrayInt& swap_order)
{
    bool found = false;

    for (int o_idx = 0; o_idx < rsite_orders.size(); ++o_idx)
    {
        ArrayInt& r_order = rsite_orders[o_idx];
...
{
    QS_DEF(ArrayInt, tmp_buf);
    ObjArray<ArrayInt>& attachment_order = match.attachmentOrder;
    ObjArray<ArrayInt>& attachment_index = match.attachmentIndex;
    ...
    ArrayInt& visited_atoms = match.visitedAtoms;
...
void IndigoDeconvolution::DecompositionEnumerator::_refineAutoMaps(ObjList<ArrayInt>& auto_maps, Graph& sub_in, Graph& super, ArrayInt& scaf_map)
{
    QS_DEF(ArrayInt, indices_to_remove);
    ...
    {
        ArrayInt& auto_map = auto_maps.at(auto_idx);
...
bool IndigoDeconvolution::DecompositionEnumerator::_cbAutoCheckAutomorphism(Graph&, const ArrayInt& mapping, const void* context)
{
    ObjList<ArrayInt>& auto_maps = *(ObjList<ArrayInt>*)context;
