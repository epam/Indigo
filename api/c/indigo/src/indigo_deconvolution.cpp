...
void IndigoDeconvolution::createRgroups(IndigoDecompositionMatch& deco_match, bool change_scaffold)
{
    ...
    for (int rg_idx = 0; rg_idx < n_rgroups; ++rg_idx)
    {
         ...
         * Add a fragment
         */
        Molecule& fragment = r_group.fragments.at(r_group.fragments.add(std::type_index(typeid(Molecule)), [] { return std::unique_ptr<BaseMolecule>(std::make_unique<Molecule>()); })).asMolecule();
