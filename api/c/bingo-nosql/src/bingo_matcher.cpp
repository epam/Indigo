...
const ArrayInt& MoleculeSubMatcher::currentMapping()
{
    return _mapping;
}
...
const ObjArray<ArrayInt>& ReactionSubMatcher::currentMapping()
{
    return _mapping;
}
...
void TopNSimMatcher::_findTopN()
{
    ...
    QS_DEF(ArrayInt, nhits_per_block);
    QS_DEF(ArrayInt, blocks);
    QS_DEF(ArrayInt, cells);
...
void TopNSimMatcher::_initModelDistribution(Array<float>& model_thrs, ArrayInt& model_nhits_per_block)
{
    for (int i = 0; i < 9; i++)
    {
