...
GrossStorageShort::GrossStorageShort(size_t gross_block_size) : _gross_formulas(gross_block_size)
{
}

MMFAddress GrossStorageShort::create(MMFPtr<GrossStorageShort>& gross_ptr, size_t gross_block_size)
{
    gross_ptr.allocate();
    new (gross_ptr.ptr()) GrossStorageShort(gross_block_size);

    return gross_ptr.getAddress();
}

void GrossStorageShort::load(MMFPtr<GrossStorageShort>& gross_ptr, MMFAddress offset)
{
    gross_ptr = MMFPtr<GrossStorageShort>(offset);
}

void GrossStorageShort::add(const Array<char>& gross_formula, int id)
{
    _gross_formulas.add((byte*)gross_formula.ptr(), gross_formula.size(), id);
    dword hash = _calculateGrossHash(gross_formula.ptr(), gross_formula.size());
    _hashes.add(hash, id);
...
void GrossStorageShort::find(Array<char>& query_formula, Array<int>& indices, int part_id, int part_count)
{
    Array<int> candidates;
...
void GrossStorageShort::findCandidates(Array<char>& query_formula, Array<int>& candidates, int part_id, int part_count)
{
    dword query_hash = _calculateGrossHash(query_formula.ptr(), query_formula.size());
...
int GrossStorageShort::findNext(Array<char>& query_formula, Array<int>& candidates, int& cur_candidate)
{
    Array<int> query_array;
    MoleculeGrossFormula::fromString(query_formula.ptr(), query_array);
...
bool GrossStorageShort::tryCandidate(Array<int>& query_array, int id)
{
    Array<int> cand_array;
    const char* cand_formula;
    int len;
...
void GrossStorageShort::calculateMolFormula(Molecule& mol, Array<char>& gross_formula)
{
    auto gross_array = MoleculeGrossFormula::collect(mol);
...
void GrossStorageShort::calculateRxnFormula(Reaction& rxn, Array<char>& gross_formula)
{
    gross_formula.clear();
...
dword GrossStorageShort::_calculateGrossHashForMolArray(Array<int>& gross_array)
{
    dword hash = 0;
...
dword GrossStorageShort::_calculateGrossHashForMol(const char* gross_str, int len)
{
    Array<int> gross_array;
    MoleculeGrossFormula::fromString(gross_str, gross_array);
...
dword GrossStorageShort::_calculateGrossHash(const char* gross_str, int len)
{
    dword hash = 0;
