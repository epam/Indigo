...
{
    const string baseTestFolder = "../../";
    // const string baseTestFolder = "../indigo-tests/";
    ...
    {
        int obj = -1;
        const string fName = baseTestFolder + "molfiles/affine/2-bromobenzenethiol-rot.mol";
        obj = indigoLoadStructureFromFile(fName.c_str(), "query");
        // EXPECT_EQ(1, indigoCheckQuery(obj));
        EXPECT_EQ(8, indigoCountAtoms(obj));
        EXPECT_EQ(8, indigoCountBonds(obj));

        const string fReactName = baseTestFolder + "/rxnfiles/epoxy.rxn";
        obj = indigoLoadStructureFromFile(fReactName.c_str(), "query");
        // EXPECT_EQ(1, indigoCheckQuery(obj));
...
TEST(IndigoLoadTest, fromGzFile)
{
    ...
    try
    {
        string fileName = "api/tests/basic/molecules/benzene.mol.gz";
        const string fullName = baseTestFolder + fileName;
        int obj = indigoLoadStructureFromFile(fullName.c_str(), "query");
        // EXPECT_EQ(1, indigoCheckQuery(obj));
    ...
    try
    {
        string fileName = "molfiles/benzodiazepine.sdf.gz";
        const string fullName = baseTestFolder + fileName;
        int obj = indigoLoadStructureFromFile(fullName.c_str(), "");
        EXPECT_EQ(33, indigoCountAtoms(obj));
        EXPECT_EQ(35, indigoCountBonds(obj));
    }
    catch (Exception& e)
    {
        ASSERT_STREQ("", e.message());
...
TEST(IndigoLoadTest, noFile)
{
    ...
    {
        ASSERT_STREQ("scanner: can't open file . Error: No such file or directory", e.message());
    }
}
