...
mol_with_pseudo = indigo.loadMoleculeFromFile(
...
def compare_names(m1, m2, reloaded_has_name):
    ...
    if reloaded_has_name and name2 == "":
        return "Reloaded object must have name %s but has empty name %s " % (
            name1,
            name2,
        )
    if not reloaded_has_name and name2 != "":
        return (
            "Reloaded object name must be empty but has %s. Source name was %s"
            % (
                name2,
                name1,
            )
...
def testSaveLoad(
    objs, filename, format, loader_func, compare_func, reloaded_has_name
):
    ...
    try:
        for i, m in zip(saved, loader):
            ...
            try:
                ...
                if err_names:
                    print(
                        "Original object #%d name doesn't match object name #%d in %s: %s"
                        % (
                            orig_obj_id,
                            i + 1,
                ...
                if err:
                    print(
                        "Original object #%d doesn't match object #%d in %s: %s"
                        % (orig_obj_id, i + 1, relativePath(filename), err)
                    )
            except IndigoException as e:
                print(
                    "Exception on compare #%d from %s with original object #%d: %s"
                    % (
                        i + 1,
                        relativePath(filename),
                        orig_obj_id,
...
def compare(m1, m2):
    ...
    if data1 != data2:
        return "Molecules representation doesn't match: %s and %s" % (
            data1,
            data2,
        )
    return None
...
def compareReactions(r1, r2):
    ...
    if data1 != data2:
        return "Reactions representation doesn't match: %s and %s" % (
            data1,
            data2,
        )
    return None
...
def compareQueryReactions(m1, m2):
    ...
    if data1 != data2:
        return "Query reactions representation doesn't match: %s and %s" % (
            data1,
            data2,
        )
    return None
...
def compareQuery(m1, m2):
    ...
    if data1 != data2:
        return "Query molecules representation doesn't match: %s and %s" % (
            data1,
            data2,
        )
    return None
...
testSaveLoad(
    mols,
    joinPathPy("savers_out/out_mols.sdf", __file__),
    "sdf",
...
testSaveLoad(
    mols,
    joinPathPy("savers_out/out_mols.smi", __file__),
    "smiles",
...
testSaveLoad(
    mols,
    joinPathPy("savers_out/out_mols_with_names.smi", __file__),
    "smiles",
    indigo.iterateSmilesFile,
    compare,
    True,
...
testSaveLoad(
    mols,
    joinPathPy("savers_out/out_mols_without_names.smi", __file__),
    "smiles",
    indigo.iterateSmilesFile,
    compare,
    False,
...
testSaveLoad(
    mols,
    joinPathPy("savers_out/out_mols.cml", __file__),
    "cml",
    indigo.iterateCMLFile,
    compare,
    True,
...
testSaveLoad(
    mols,
    joinPathPy("savers_out/out_mols.rdf", __file__),
    "rdf",
...
testSaveLoad(
    qmols,
    joinPathPy("savers_out/out_qmols.rdf", __file__),
    "rdf",
    indigo.iterateRDFile,
    compareQuery,
    True,
...
testSaveLoad(
    reacts,
    joinPathPy("savers_out/out_reacts.rdf", __file__),
    "rdf",
    indigo.iterateRDFile,
    compareReactions,
    True,
...
testSaveLoad(
    reacts,
    joinPathPy("savers_out/out_reacts_without_names.smi", __file__),
    "smi",
    indigo.iterateSmilesFile,
    compareReactions,
    False,
...
testSaveLoad(
    reacts,
    joinPathPy("savers_out/out_reacts_with_names.smi", __file__),
    "smi",
    indigo.iterateSmilesFile,
    compareReactions,
    True,
...
testSaveLoad(
    qreacts,
    joinPathPy("savers_out/out_qreacts.rdf", __file__),
    "rdf",
    indigo.iterateRDFile,
    compareQueryReactions,
    True,
)
