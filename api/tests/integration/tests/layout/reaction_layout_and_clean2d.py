...
saver_layout = indigo.writeFile(joinPathPy("out/reaction_layout.rdf", __file__))
saver_clean = indigo.writeFile(joinPathPy("out/reaction_clean2d.rdf", __file__))
...
for idx, item in enumerate(indigo.iterateRDFile(joinPathPy("reactions/reaction_layout_and_clean2d.rdf", __file__))):
    try:
        rxn = item.clone()
        rxn.layout()
