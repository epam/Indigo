...
def testTopNWithExtFP(bingo, q, limit):
    ...
    ids, sims = getTuple(
        bingo.searchSimTopNWithExtFP(q, limit, 0.3, fp, "tanimoto")
    )
...
bingo = Bingo.createDatabaseFile(
    indigo, joinPathPy("out/db_molecule_sim", __file__), "molecule", ""
)
...
for index, mol in enumerate(
    indigo.iterateSmilesFile(
        joinPathPy("molecules/sample_2000_1.smi", __file__)
    )
):
    try:
...
for index, mol in enumerate(
    indigo.iterateSmilesFile(joinPathPy("molecules/sample_200.smi", __file__))
):
    print("** Query %d: %s" % (index, mol.smiles()))
...
bingo = Bingo.createDatabaseFile(
    indigo, joinPathPy("out/db_molecule_sim_small", __file__), "molecule", ""
)
bingo.insert(
    indigo.loadMolecule(
        "Fc1cccc(NC(c2ccc(NC(C(C)N3CCC(C(N4CCCC4C(Nc4cccc(C(Nc5ccccc5)=O)c4)=O)=O)CC3)=O)c(C)c2)=O)c1"
    )
)
