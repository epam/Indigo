...
def searchGross(bingo, q, requiredId = None, options=''):
    ...
    while result.next():
        try:
            ...
            print('\t{0} {1} {2}. {3}'.format(result.getCurrentId(), rm.smiles(), mol.smiles(), mol.grossFormula()))
    ...
    if requiredId is not None and not requiredIdFound:
        print("Error: cannot find molecule by the same molFormula: {0} by id={1}".format(q, requiredId))
