...
def paralBigTest():
    ...
    for i in range(0, 5):
        thrs.append(threading.Thread(target=insertThr, args=(bingo1, i, [i % 5 + 1])))
    for i in range(5, 10):
        thrs.append(threading.Thread(target=insertThr, args=(bingo2, i, [i % 5 + 1])))
    ...
    for bingo in [bingo1, bingo2]:
       print('Database #{0}'.format(cnt))
       off = (cnt - 1) * 5
       thrs.append(threading.Thread(target=makeSearchSim, args=(bingo, 0 + off, query, 0.7, 1, 'tanimoto', thr_outputs)))
       thrs.append(threading.Thread(target=makeSearchSim, args=(bingo, 1 + off, query, 0.7, 1, 'tversky 0.2 0.8', thr_outputs)))
       thrs.append(threading.Thread(target=makeSearchSim, args=(bingo, 2 + off, query, 0.7, 1, 'euclid-sub', thr_outputs)))
