...
def insertProc(db, pid, input_smi, output=None):
    ...
    outPrint('Finished indexing {1} structures. {2} wrong structures excluded'.format(pid, index, wrongStructures), pid,
             output)
...
def paralSameTest():
    ...
    for i in range(1, 51):
        thrs.insert(i - 1, threading.Thread(target=insertThr, args=(bingo1, [(i - 1) % 5 + 1])))
...
def paralBigTest():
    ...
    for i in range(0, 5):
        thrs.append(threading.Thread(target=insertThr, args=(bingo1, i, [i % 5 + 1], outp)))
    for i in range(5, 10):
        thrs.append(threading.Thread(target=insertThr, args=(bingo2, i, [i % 5 + 1], outp)))

    for i in range(10, 20):
        thrs.append(threading.Thread(target=makeSearchSim, args=(bingo1, i, query, 0.7, 1, 'tanimoto', outp)))
    for i in range(20, 30):
        thrs.append(threading.Thread(target=makeSearchSub, args=(bingo1, i, q_query, '', outp)))
    for i in range(30, 40):
        thrs.append(threading.Thread(target=makeSearchExact, args=(bingo1, i, query, '', outp)))

    for i in range(40, 50):
        thrs.append(threading.Thread(target=makeSearchSim, args=(bingo2, i, query, 0.7, 1, 'tanimoto', outp)))
    for i in range(50, 60):
        thrs.append(threading.Thread(target=makeSearchSub, args=(bingo2, i, q_query, '', outp)))
    for i in range(60, 70):
        thrs.append(threading.Thread(target=makeSearchExact, args=(bingo2, i, query, '', outp)))
    ...
    for bingo in [bingo1, bingo2]:
        ...
        off = (cnt - 1) * 5
        thrs.append(
            threading.Thread(target=makeSearchSim, args=(bingo, 0 + off, query, 0.7, 1, 'tanimoto', thr_outputs)))
        thrs.append(threading.Thread(target=makeSearchSim,
                                     args=(bingo, 1 + off, query, 0.7, 1, 'tversky 0.2 0.8', thr_outputs)))
        thrs.append(
            threading.Thread(target=makeSearchSim, args=(bingo, 2 + off, query, 0.7, 1, 'euclid-sub', thr_outputs)))
        thrs.append(threading.Thread(target=makeSearchSub, args=(bingo, 3 + off, q_query, '', thr_outputs)))
        thrs.append(threading.Thread(target=makeSearchExact, args=(bingo, 4 + off, query, '', thr_outputs)))
        cnt = cnt + 1
