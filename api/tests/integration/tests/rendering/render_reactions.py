...
def renderRxnfile(filename, outfile):
    ...
    indigo.setOption("render-output-format", "svg")
    renderer.renderToFile(rxn2, joinPathPy("out/%s-smiles.svg" % (outfile), __file__))
    print(checkImageSimilarity('%s.svg' % outfile))
    indigo.setOption("render-output-format", "png")
    renderer.renderToFile(rxn2, joinPathPy("out/%s-smiles.png" % (outfile), __file__))
    print(checkImageSimilarity('%s.png' % outfile))
    ...
    renderer.renderToFile(rxn3, joinPathPy("out/%s.cdxml" % (outfile), __file__))


indigo.setOption("render-output-format", "png")
for size in range(10, 100, 5):
    indigo.setOption("render-bond-length", "%f" % (size))
    m = indigo.loadReactionFromFile(joinPathPy("reactions/adama_reaction.rxn", __file__))
    renderer.renderToFile(m, joinPathPy("out/adama_reaction-size-%03d.png" % (size), __file__))
