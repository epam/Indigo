...
tests = [
   ...
   (lmol('N\\C=C\\C'), lqmol("[H,*]\\C=C\\N")),
   (lmol('N\\C=C\\C'), lqmol("[H,*]\\C=C/N")),
   (lmolf(joinPath('molecules/pseudo_target.mol')), 
        lqmolf(joinPath("molecules/pseudo_query.mol"))),
   (lmolf(joinPath('molecules/pseudo_target2.mol')), 
        lqmolf(joinPath("molecules/pseudo_query.mol"))),
   (lmolf(joinPath('molecules/test_basic_smarts.mol')), lsmarts("[Fe]~N")),
   (lmolf(joinPath('molecules/test_basic_smarts.mol')), lsmarts("[Fe]~[N]")),
   (lmolf(joinPath('molecules/test_basic_smarts.mol')), lsmarts("[Fe]N")),
   (lmolf(joinPath('molecules/test_basic_smarts.mol')), lsmarts("[Fe]-N")),
   (lmolf(joinPath('molecules/test_basic_smarts.mol')), lsmarts("[Fe]-[#7]")),
   (lmolf(joinPath('molecules/bad_valance.mol')), lsmarts("[#7&v5]=O")),
