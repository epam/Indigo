...
def test_iterate_cml():
    results = []
    for step in range(0, 2):
        if 0 == step:
            cml = helpers.iterate_cml("resources/tetrahedral-all.cml")
        else:
            cml = helpers.iterate_file(Path("resources/tetrahedral-all.cml"))
        i = 0
