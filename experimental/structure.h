...
namespace indigo2
{
    ...
    class JsonStream : public iostream
    {
        JSonStream& operator<<(std::pair<std::string, TYPE> namevalue)
        {
            iostream::operator<<(*this, "\"");
            iostream::operator<<(*this, namevalue.first);
            iostream::operator<<(*this, "\"");
            iostream::operator<<(*this, "=");
            iostream::operator<<(*this, namevalue.second);
            iostream::operator<<(*this, ",");
            return *this;
        }
    };
...
#define SERIALIZE_FIELD(NAME)                                                                                                                                  \
    if (os)                                                                                                                                                    \
    {                                                                                                                                                          \
        os << std::pair<std::string, TYPE>(##NAME, this.NAME);                                                                                                 \
    }                                                                                                                                                          \
    ...
    {
    public:
        void serialize(ostream& os){os << } 
        void unserialize(istream& is)
        {
        }

    protected:
        virtual void _scan_members(std::istream* is, std::ostream* os, const std::string& name) = 0;
    };

    typedef Vertice shared_ptr<_Vertice>;
    class _Vertice : public StrucutreItem
    {
        Vec3f coord;
    };

    typedef Edge shared_ptr<_Edge>;
    class _Edge : public StructureItem
    {
        Vertice beg;
        Vertice end;
    };

    typedef Structure shared_ptr<_Structure>;
    class _Structure : public Vertice
    {
        map<ID, Vertice> vertices;
        map<ID, Edge> edges;
    };
