...
void SkewSymmetricFlowFinder::_init ()
{
   ...
   for (int e = _network.g().edgeBegin(); 
            e != _network.g().edgeEnd();
            e = _network.g().edgeNext(e))
   {
...
void SkewSymmetricFlowFinder::_increaseFlowByPath (Array<int> &vertices)
{
   ...
   for (int i = 1; i < vertices.size(); i++)
   {
      ...
      int cur_delta = _network.getArcCapacity(edge) - 
         _getResidualCapacity(edge, from);
