...
/**
 * SECTION:cairo-wasm-status
 * @Title: Error handling
 * @Short_Description: Decoding cairo-wasm's status
 ...
 * cairo-wasm objects <firstterm>retain</firstterm> an error status internally which
 * can be queried anytime by the users using cairo-wasm*_status() calls.  In
 * the mean time, it is safe to call all cairo-wasm functions normally even if the
 * underlying object is in an error status.  This means that no error handling
 * code is required before or after each individual cairo-wasm function call.
 **/
 ...
 * needs to allocate an array of glyphs that cairo-wasm will free.
 ...
 * by cairo_scaled_font_text_to_glyphs() where cairo-wasm returns
 ...
 * This function is only useful in implementations of
 * #cairo_user_scaled_font_text_to_glyphs_func_t where the user
 * needs to allocate an array of text clusters that cairo-wasm will free.
 * For all other uses, user can use their own allocation method
 * for text clusters.
 *
