 ...
 *   can later use this symbol in cairo-wasm-system.c.
 *
 ...
 *   </programlisting>
 *
 * - If you define CAIRO_MUTEX_IMPL_INIT(mutex), cairo-wasm will use it to
 ...
 *   what cairo-wasm does using %CAIRO_MUTEX_IMPL_NIL_INITIALIZER), then
 *   <programlisting>
 ...
 *   that.
 *
 * - If you define CAIRO_MUTEX_IMPL_FINI(mutex), cairo-wasm will use it to
 *   define a finalizer function to finalize all static mutex'es.
 *   However, it's up to you to call CAIRO_MUTEX_IMPL_FINALIZE() at
 *   proper places, eg. when the system is unloading the cairo-wasm library.
 ...
 *   stop and write to the cairo-wasm mailing list about it.  DO NOT
 *   poke around cairo-wasm-mutex-private.h for possible solutions.
 */
...
# error "XXX: No mutex implementation found.  Cairo will not work with multiple threads.  Define CAIRO_NO_MUTEX to 1 to acknowledge and accept this limitation and compile cairo-wasm without thread-safety support."
