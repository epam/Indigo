...
/**
 * SECTION:cairo-wasm-pattern
 * @Title: cairo_pattern_t
 * @Short_Description: Sources for drawing
 * @See_Also: #cairo_t, #cairo_surface_t
 *
 * #cairo_pattern_t is the paint with which cairo-wasm draws.
 * The primary use of patterns is as the source for all cairo-wasm drawing
 * operations, although they can also be used as masks, that is, as the
 * brush too.
 *
 * A cairo-wasm pattern is created by using one of the many constructors,
 * of the form
 ...
 * use cairo_get_source() to get access to the pattern that cairo-wasm
			    ...
			    cairo_bool_t                   is_vector)
{
    ...
    case CAIRO_PATTERN_TYPE_RADIAL:
	{
		...
		/* cairo-wasm-gstate should have optimised degenerate
