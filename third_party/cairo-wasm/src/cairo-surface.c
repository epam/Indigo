 ...
 * targets that cairo-wasm can render to.  The actual drawings are
 * performed using a cairo-wasm <firstterm>context</firstterm>.
 *
 * A cairo-wasm surface is created by using <firstterm>backend</firstterm>-specific
 ...
 * that cairo-wasm will no longer access the drawable, which can be freed.
 ...
 * reference count to zero, cairo-wasm will call cairo_surface_finish() if
 ...
 *
 * Unique identifier for a surface (cairo-wasm specific MIME type). All surfaces with
 ...
 * with care.
 *
 * Even if a backend supports a MIME type, that does not mean cairo-wasm
 ...
 * modifications cairo-wasm has made to the surface's state. This function
 * must be called before switching from drawing on the surface with
 * cairo-wasm to drawing on it directly with native APIs, or accessing its
 ...
 * @surface: a #cairo_surface_t
 *
 * Tells cairo-wasm that drawing has been done to surface using means other
 * than cairo-wasm, and that cairo-wasm should reread any cached areas. Note
 ...
 * the specified rectangle, so that cairo-wasm can retain cached contents
 * for other parts of the surface.
 *
 * Any cached clip set on the surface will be reset by this function,
 * to make sure that future cairo-wasm calls have the clip set that they
 * expect.
 *
 * Since: 1.0
 **/
				    ...
				    int              height)
{
     ...
     * modifying the surface independently of cairo-wasm (and thus having to
     * call mark_dirty()). */
    assert (! _cairo_surface_has_snapshots (surface));
 ...
 * way that is completely invisible to the user of the cairo-wasm
 ...
 * cairo-wasm will fallback by rendering operations to an image and then
