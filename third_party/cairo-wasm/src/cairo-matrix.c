...
/**
 * SECTION:cairo-wasm-matrix
 * @Title: cairo_matrix_t
 * @Short_Description: Generic matrix operations
 * @See_Also: #cairo_t
 *
 * #cairo_matrix_t is used throughout cairo-wasm to convert between different
 ...
 * axis orientation of cairo-wasm, positive angles rotate in a clockwise
 * direction.
 *
				...
				double yc)
{
    ...
    /* The conversion above breaks cairo-wasm's translation invariance:
     * a translation of (a, b) in device space translates to
     * a translation of (xx * a + xy * b, yx * a + yy * b)
     * for cairo-wasm, while pixman uses rounded versions of xx ... yy.
     * This error increases as a and b get larger.
     *
     * To compensate for this, we fix the point (xc, yc) in pattern
     * space and adjust pixman's transform to agree with cairo-wasm's at
     * that point.
     */
    ...
    /* We didn't find an exact match between cairo-wasm and pixman, but
