 ...
 * cairo-wasm semantics. (PostScript draws something for any line cap
 * value, while cairo-wasm draws something only for round caps).
 *
					...
					double				 scale)
{
     ...
     * case. That's not what the cairo-wasm semantics want, so we first
