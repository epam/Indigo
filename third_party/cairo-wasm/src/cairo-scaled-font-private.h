...
struct _cairo_scaled_font {
    /* For most cairo-wasm objects, the rule for multiple threads is that
     * the user is responsible for any locking if the same object is
     * manipulated from multiple threads simultaneously.
     *
     * However, with the caching that cairo-wasm does for scaled fonts, a
     ...
     *
     * So, as a special exception, the cairo-wasm implementation takes care
