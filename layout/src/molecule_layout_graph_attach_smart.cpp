...
void MoleculeLayoutGraphSmart::_setChainType (const Array<int> &chain, const Array<int> &mapping, int type)
{
...
//         if (!next_bc._isPointOutside(next_bc._layout_vertices[mapping[chain_ext[1]]].pos))
  //          continue;
      ...
      if (n_try == 0)
      {
         for (int i = 0; i < border.vertexCount(); i++) if (border.getVertex(i) != c_beg && border.getVertex(i) != c_end)
            if (next_bc._isPointOutsideCycleEx(cycle, next_bc._layout_vertices[mapping[border.getVertex(i)]].pos, mapping))
               is_attached = true;
